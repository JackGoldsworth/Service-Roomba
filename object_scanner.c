...
Object *sweep(void) {
    ...
    while (degree < 181) {
        ...
        if (adc_distance <= range && objArray[objectIndex].startAngle == 0 && degree > 3) {
            ...
            obj.width = 2 * (minimumDistance / currentDistanceIndex) *
                        tan(((obj.endAngle - obj.startAngle + 0.2) / 2) * M_PI / 200);
            objArray[objectIndex] = obj;
            objectIndex++;
        ...
        if (objectIndex - 1 != -1) {
            lcd_printf("Current Index: %d \nCurrent Distance: %dWidth: %d", objectIndex,
                       (int) objArray[objectIndex - 1].distance, (int) objArray[objectIndex - 1].width);
        }
    ...
    if (last.startAngle != 0 && last.endAngle == 0) {
        ...
        last.width = 2 * (minimumDistance / currentDistanceIndex) *
                     tan(((last.endAngle - last.startAngle + 0.2) / 2) * M_PI / 200);
...
//            char string[] = "Found Object!\rDistance: " + objArray[i].distance + "\rWidth: " + objArray[i].width + "\rAngle: " + objArray[i].middleValue + "\r";
            char string[50];
            snprintf(string, 50, "Found Object!\rDistance: %d\rWidth: %d\rAngle: %d\r\r", (int) objArray[i].distance,
                     (int) objArray[i].width, (int) objArray[i].middleValue);
            uart_sendStr(string);
        }
    }
    return objArray;
